...
python3 typer_commands.py upload-directory ${TEAM_ID} src/download/ds1/img/ /uploaddir/${TASK_ID}-${PROJECT_ID}/ && \
# && \

# supervisely (cli) set-task-output-dir ${TASK_ID} "/my-training/{TASK_id}{PROJECT_NAME}-{PROJECT_ID}"
python3 typer_commands.py set-task-output-dir ${TASK_ID} /uploaddir/${TASK_ID}-${PROJECT_ID}/